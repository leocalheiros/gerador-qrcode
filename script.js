...
const linkQrCode = document.querySelector('#linkQrCode');
...
btnValue.addEventListener('click', () => {
  ...
  imgQrCode.src = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(qrcodeValue)}`;
  imgQrCode.addEventListener('load', () => {
    wrapper.classList.add('active');
    btnValue.innerText = 'Gerar QRCode';
    linkQrCode.value = imgQrCode.src;
  });
});
